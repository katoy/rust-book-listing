...
pub fn run_game<R: BufRead, W: Write>(reader: &mut R, writer: &mut W) -> io::Result<()> {
    writeln!(writer, "数当ててみぃや！")?; // Guess the number!
    writeln!(writer, "ほな、予想入れてみて！")?; // Please input your guess.
    ...
    match parse_guess(&guess) {
        Some(num) => writeln!(writer, "あんたの予想は {} やな！", num)?, // You guessed: {}
        None => writeln!(writer, "ちゃんとした数字入れてや！")?, // Please enter a valid number!
    }
